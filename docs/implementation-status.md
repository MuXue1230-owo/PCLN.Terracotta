...
- **alpha.2：** `EasyTierRoomBackend`、房间凭据、EasyTier sidecar、Scaffolding、本机转发与 discovery；`network.easytier-missing` 等稳定错误码。
- **alpha.3：** 确定性 mesh 端点、房主 mesh ingress、成员 EasyTier `--port-forward` 跨机路径、同机 discovery 优先回退、可选 `TERRACOTTA_EASYTIER_ALLOW_TUN`；`network.mesh-ingress-failed` / `TC-NET-005`。

## 正在推进

- NAT 探测与直连/中继质量的 RPC 细化；
- 六 RID `easytier-core` 制品纳入 `.pnp` 与 CI。

## 后续里程碑

1. EasyTier RPC 诊断（NAT、RTT、relay 节点）与 UI 网络质量展示；
2. 六 RID `easytier-core` 与 Helper 一并签名打包；
3. 解决公共 Contracts 的默认 ALC 共享机制并注册四个插件导出；
4. 跨 PCL N、PCL CE 和其他兼容启动器的端到端联机测试；
5. 完整许可证、SBOM、第三方许可、官方签名和商店审核发布。
